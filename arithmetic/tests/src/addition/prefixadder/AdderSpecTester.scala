...
object AdderSpecTester extends FormalSuite {
  ...
  val tests: Tests = Tests {
    test("ripple carry should pass") {
      verify(new RippleCarryAdder(width), Seq(BoundedCheck(0)))
    }
    test("ripple carry 3 fan-in should pass") {
      verify(new RippleCarry3Adder(width), Seq(BoundedCheck(0)))
    }
    test("kogge stone should pass") {
      verify(new KoggeStoneAdder(width), Seq(BoundedCheck(0)))
    }
    test("brent kung should pass") {
      verify(new BrentKungAdder(width), Seq(BoundedCheck(0)))
   }
  }
}
