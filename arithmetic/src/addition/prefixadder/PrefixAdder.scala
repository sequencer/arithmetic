...
class UnsignedPrefixAdder(val width: Int, prefixSum: PrefixSum) extends UnsignedFullAdder {
  ...
  val cs: Vector[Bool] = cin +: pgs.map(_._2) // Include carry-in of 0
...
class SignedPrefixAdder(val width: Int, prefixSum: PrefixSum) extends SignedFullAdder {
  ...
  // Recombine bits into bitvector
  z := VecInit(sum).asUInt.asSInt
}

// // a和b具有不同位宽时的符号扩展应该在哪里进行
// class FixedPointPrefixAdder(val width: Int, val BPWidth: Int, prefixSum: PrefixSum) extends FixedPointFullAdder {
//   override val desiredName: String = this.getClass.getSimpleName + width.toString
...
//   // Recombine bits into bitvector
//   z := VecInit(sum).FixedPoint((width+1).W, BPWidth.BP)
// }
